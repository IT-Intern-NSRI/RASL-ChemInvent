...
| Current Inventory / For Purchase as quantity expressions | Both are free text in the same `<count> x <amount><unit>[, ...]` grammar as Quarterly Stocking Qty (e.g. `"1 x 1kg, 1 x 250g"`) - `QuantityExpressionInput.tsx` (single text field + a structured-builder popover), stored via `QuarterEntry.currentInventory`/`.forPurchase` (`String?`, migration `20260728120000_quantity_expressions`) |
